...
このチュートリアルは、`cor.table.jap` の主要機能を**再現可能なダミーデータ**で実演します。
...
> このノートブックは `set.seed()` を使うため、同じ結果が再現できます。

## セットアップ

公開版は `renv.lock` で再現できるため、必要に応じて `renv::restore()` を先に実行してください。

```{r}
# パッケージ読み込み
# 公開用: GitHub からインストール（再現性のため固定コミットを指定）
...
臨床心理学でよく使われる尺度名（略称）を使ったダミーデータを作成します。
...
# 臨床心理学で馴染みのある尺度名（略称）
# 典型的な関係: FFMQ は STAI/PSS/DASS と負相関、STAI/PSS/DASS は正相関
df <- data.frame(
